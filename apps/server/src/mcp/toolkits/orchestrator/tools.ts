...
const OrchestratorCapabilitiesTool = Tool.make("orchestrator_capabilities", {
  description:
    "List the V2 provider instances and the models allowed for subagents on each, including configured custom models. That allowlist is independent of the composer model picker. Also returns inherited runtime settings and app-owned orchestration features available to this T3 thread. For a separate top-level thread in a new or existing worktree, use t3_thread_launch with workspaceStrategy.",
